class BuildingTile extends "Tile.gd".Tile:
	...
	func _init(remaining:int, level:int, max_level:int, level_cost:int, name: String, description: String, type:String, position:Vector3, texture:int, owner:String, status:String).(name, description, type, position, texture, owner, status):
		# this class vars
